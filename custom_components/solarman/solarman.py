...
class Inverter:
    def __init__(self, path, serial, host, port, mb_slaveid, tcp, lookup_file):
        self._modbus = None
        self._serial = serial
        self.path = path
        self._host = host
    ...
    def connect_to_server(self):
        ...
        if self._tcp:
            self._modbus = ModbusClient(host=self._host, port=self._port, timeout=15, auto_open=True, auto_close=True)
        else:
    ...
    def disconnect_from_server(self):
        if self._modbus and not self._tcp:
            try:
    ...
    def send_request(self, params, start, end, mb_fc):
        if self._tcp:
            self.send_request_tcp(params, start, end)
        else:
            self.send_request_rtu(params, start, end, mb_fc)

    def send_request_rtu(self, params, start, end, mb_fc):
        length = end - start + 1
        match mb_fc:
            case 3:
    ...
    def send_request_tcp(self, params, start, end):
        length = end - start + 1
        response = self._modbus.read_holding_registers(start, length)
        params.parse(response, start, length)
...
# Service calls
    def service_write_holding_register(self, register, value):
        ...
        try:
            if self._tcp:
                self._modbus.write_single_register(register, value)
            else:
                self.connect_to_server()
    ...
    def service_write_multiple_holding_registers(self, register, values):
        ...
        try:
            if self._tcp:
                self._modbus.write_multiple_registers(register, values)
            else:
                self.connect_to_server()
